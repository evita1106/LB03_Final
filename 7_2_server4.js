/* *******************************************************************************************
* Autor: E. Rauber,A.Treichler, 1/2024
* *******************************************************************************************
* Beschreibung:
...
connection.connect(function(err) {
    ...
    var sqlstmt = 'SELECT * from students';
...
app.get('/Students', (req, res) => {
    connection.query('SELECT * FROM students', [req.params.id], (err, rows, fields) => {
        if (!err) {
            console.log(rows);
            res.send(rows);
        } else {
...
app.get('/Students/:id', (req, res) => {
    connection.query('SELECT * FROM Students WHERE Students_ID = ?', [req.params.id], (err, rows, fields) => {
        if (!err) {
            console.log(rows);
            res.send(rows);
        } else {
...
app.delete('/Students/:Student_ID', (req, res) => {
    connection.query(' DELETE FROM Students WHERE Student_ID = ? ', [req.params.id], (err, rows, fields) => {
        if (!err) {
